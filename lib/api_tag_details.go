...
func (r *registryApi) GetTagDetails(ctx context.Context, ref Refspec, manifestVersion uint) (details TagDetails, err error) {
	...
	//fmt.Println(string(bodyBuffer.Bytes()))
...
func (r *registryApi) ManifestDigest(ref Refspec, manifestVersion uint, repository, reference string) (digest.Digest, error) {
	url := r.endpointUrl(fmt.Sprintf("/v2/%s/manifests/%s", repository, reference))
	headers, err := r.getHeadersForManifestVersion(manifestVersion)
	if err != nil {
		return "", err
	}
	resp, err := r.connector.Head(context.TODO(), url, headers, "")
	if resp != nil {
