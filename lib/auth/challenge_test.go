...
func TestSimple(t *testing.T) {
	//testcase := `Bearer realm="https://docker.1ms.run/openapi/v1/auth/token", service="docker.1ms.run", scope="repository:linkease/linkease:pull"`
...
func TestHaveSpace(t *testing.T) {
	...
	fixtureUrl, _ := url.Parse("https://docker.1ms.run/openapi/v1/auth/token")
	testParse(t, testcase, Challenge{
		realm:   fixtureUrl,
		service: "docker.1ms.run",
		scope: []string{
			"repository:linkease/linkease:pull",
		},
	})
}

func TestHaveInvalidToken(t *testing.T) {
	testcase := `Bearer realm="https://registry.istoreos.com/token",service="registry.docker.io",scope="repository:linkease/linkease:pull",error="invalid_token"`
	fixtureUrl, _ := url.Parse("https://registry.istoreos.com/token")
	testParse(t, testcase, Challenge{
		realm:   fixtureUrl,
		service: "registry.docker.io",
		scope: []string{
